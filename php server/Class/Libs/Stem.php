<?php
      class Stem {
          ...
          function fordata($string, $implode = true){
            $r = Array();
            $words = explode(" ", preg_replace("/\b\w{0,2}\b/ui", '', preg_replace("/[^a-zA-Zа-яА-Я\s\s+]/ui", '', str_replace('-', ' ', $string))));
            if($cc = count($words))
              for ($j=0; $j <= $cc-1; $j++) { 
                $r[$this->stem_word($words[$j])] = 1;
              }
            return $implode ? implode(' ', array_keys($r)):array_keys($r);
          }

          function stem_phrase($phrase){
              $stemmed = [];
              foreach (explode(' ', $phrase) as $word) {
                  $stemmed[] = $this->stem_word($word);
              }
              unset($phrase);
              unset($word);
